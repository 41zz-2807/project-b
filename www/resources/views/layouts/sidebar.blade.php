...
<aside id="sidebar"
    ...
    <div class="pt-8 pb-7 flex items-center gap-2" :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ? 'justify-center' : 'justify-between'">
        <a href="/">
            <div class="hidden [.sidebar-expanded_&]:block min-w-0 text-gray-800 dark:text-white/90">
                <p class="truncate text-title-sm font-semibold leading-tight">
                    {{ setting('nama_sekolah') ?: 'Bendahara Kas' }}
                </p>
                <p class="mt-0.5 truncate text-theme-xs text-gray-500 dark:text-gray-400">
                    {{ setting('nama_kelas') ? 'Kelas ' . setting('nama_kelas') : 'Kelas' }}
                </p>
            </div>
