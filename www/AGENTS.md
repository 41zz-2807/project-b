...
## Agent Search & Read Scope

- Never read, glob, grep over, or directory-list (`ls -R`) the generated / third-party folders below. Treat them as unneeded for building or understanding this app unless the user is explicitly debugging a vendor/bundle/dependency issue:
  - `node_modules/`
  - `vendor/`
  - `public/build/`
  - `storage/framework/`
  ...
  - `bootstrap/cache/`
  - `.git/`
- Use pattern-based file discovery only (e.g. `resources/**/*.blade.php`, `app/**/*.php`), never whole-repo scans.
- Source of truth for app logic lives in: `app/`, `routes/`, `database/`, `config/`, `resources/views/`, `resources/js/`, `resources/css/`.
